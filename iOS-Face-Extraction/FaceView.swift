...
class FaceView: UIView {
	...
	func initialize(width: CGFloat, height: CGFloat) {
		...
		cameraLayer.position = CGPoint(x: frame.width / 2 , y: frame.height - 30)
	...
	func updateImage(_ image: CIImage) {
		...
		let mouthPoxY = face.mouthPosition.y
		...
		RUTools.runOnMainThread {
			...
			self.cameraLayer.anchorPoint = CGPoint(x: mouthPosX / self.captureWidth, y: (self.captureHeight - mouthPoxY) / self.captureHeight)
